...
export default class File implements IFile {
  ...
  constructor(file: { name: string; parentPath: string }) {
    ...
    if(this.type === "src") {
      ...
      this.src.absolutePath = path.join(this.original.parentPath, this.original.name);
    } else {
    ...
    this.test.absolutePath = path.join(
      Context.testFolder,
      this.test.name
    );
  }
  ...
  async loadContent() {
    const fileContent = await fs.readFileSync(this.original.absolutePath, "utf8");
  ...
  async persistPromptFile(prompt: string) {
    ...
    try {
      ...
      throw new Error(`Error writing prompt file "${this.prompt.absolutePath}": ${(e as Error).message}`);
    }
  }
  ...
  async persistSrcFile(code: string) {
    ...
    try {
      ...
      throw new Error(`Error writing src file "${this.src.absolutePath}": ${(e as Error).message}`);
    }
  }
  ...
  async persistTestFile(test: string) {
     try {
      ...
      throw new Error(`Error writing test file "${this.test.absolutePath}": ${(e as Error).message}`);
    }
  }
